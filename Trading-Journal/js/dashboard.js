window.JournalDashboard = (() => {
  ...
  function aggregateDay(trades, date) {
    ...
    return { items, pnl: items.reduce((s,t)=>s+Number(t.pnl||0),0) };
  }
      ...
      row.innerHTML=`
        <div>
          <div class="trade-main">
            ...
            <strong class="${Number(t.pnl)>0?"positive-text":Number(t.pnl)<0?"negative-text":""}">${fmtMoney(t.pnl)}</strong>
  ...
  function renderConfluences(state) {
    ...
    state.tradeConfluences.forEach(tc=>{
      ...
      const g=groups[conf.name]; g.total++; g.pnl+=Number(trade.pnl||0);
  ...
  return { renderAll, initNav, fmtMoney, iso, getSelectedDate:()=>selectedDate, setSelectedDate:d=>selectedDate=d };
})();
